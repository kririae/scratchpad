...
def is_inside(i: int, j: int, k: int, length: Any, width: Any, height: Any) -> bool:
    return (
        float(i) >= 0
        and float(i) < float(length)
        and float(j) >= 0
        and float(j) < float(width)
        and float(k) >= 0
        and float(k) < float(height)
    )
...
def get_bspline_coeff(p: wp.vec3):
    base = wp.vec3(wp.floor(p.x), wp.floor(p.y), wp.floor(p.z)) + wp.vec3(0.5, 0.5, 0.5)
    ...
    nox = wp.vec3(
        0.5 * (1.5 - ox.x) * (1.5 - ox.x),
        0.75 - ox.y * ox.y,
        0.5 * (1.5 - ox.z) * (1.5 - ox.z),
    )
    noy = wp.vec3(
        0.5 * (1.5 - oy.x) * (1.5 - oy.x),
        0.75 - oy.y * oy.y,
        0.5 * (1.5 - oy.z) * (1.5 - oy.z),
    )
    noz = wp.vec3(
        0.5 * (1.5 - oz.x) * (1.5 - oz.x),
        0.75 - oz.y * oz.y,
        0.5 * (1.5 - oz.z) * (1.5 - oz.z),
    )
...
class AffineParticleInCellSolver3D:
    def __init__(
        self, length: int, width: int, height: int, viscosity: float = 0.0
    ) -> None:
        wp.init()
    ...
    def _init(self) -> np.ndarray:
        ...
        res = np.vstack((grid_x.flatten(), grid_y.flatten(), grid_z.flatten())).T
        return res + np.random.rand(*res.shape) - 0.5

    def _init_grid(self) -> None:
        self.fg_.fill_(GAS)
        wp.launch(
            self._init_grid_kernel, dim=self.shape_, inputs=[self.fg_, self.boundary_]
        )

    def _set_flag(self) -> None:
        wp.launch(self._set_all_flag, dim=self.shape_, inputs=[self.fg_])
        wp.launch(
            self._set_fluid_flag, dim=self.p_.shape[0], inputs=[self.p_, self.fg_]
        )
    ...
    def _p2g(self) -> None:
        ...
        self.mg_.fill_(0.0)
        wp.launch(
            self._p2g_apic_kernel,
            dim=self.p_.shape[0],
            inputs=[self.u_, self.p_, self.c_, self.ug_, self.mg_],
        )
        wp.launch(
            self._p2g_postprocess_kernel,
            dim=self.shape_,
    ...
    def _g_diffuse_iter(self) -> None:
        for _ in range(self.n_diffuse_):
            wp.launch(
                self._g_diffuse_kernel,
                dim=self.shape_,
                inputs=[self.ug0_, self.ug1_, self.ug_, self.fg_, self.viscosity_],
            )
    ...
    def _g_project(self) -> None:
        self.div_.fill_(0.0)
        wp.launch(
            self._g_force_and_boundary_kernel,
            dim=self.shape_,
            inputs=[self.ug_, self.fg_, self.boundary_, self.dt_],
        )
        if False:
        ...
        wp.launch(
            self._g_calc_divergence,
            dim=self.shape_,
            inputs=[self.ug0_, self.div_, self.fg_],
        )
        wp.launch(self._g_precondition, dim=self.shape_, inputs=[self.j0_, self.fg_])
        wp.capture_launch(self.project_graph_)
        wp.launch(
            self._g_subtract_gradient_q_kernel,
            dim=self.shape_,
            inputs=[
                self.ug0_,
                self.j0_,
    ...
    def _init_grid_kernel(fg: wp.array3d(dtype=wp.uint8), boundary: int) -> None:
        ...
        if (
            (i < boundary or i >= length - boundary)
            or (j < boundary or j >= width - boundary)
            or (k < boundary or k >= height - boundary)
        ):
            fg[i, j, k] = SOLID
    ...
    @wp.kernel
    def _set_all_flag(fg: wp.array3d(dtype=wp.uint8)) -> None:
        i, j, k = wp.tid()
        if fg[i, j, k] != SOLID:
            fg[i, j, k] = GAS
        ...
        for x in range(3):
            for y in range(3):
                for z in range(3):
                    ...
                    dpos = (
                        wp.vec3(float(bi) + 0.5, float(bj) + 0.5, float(bk) + 0.5) - pp
                    )
    ...
    def _g_project_kernel(
        ...
        q_dst[i, j, k] = (1.0 - damping) * q_src[i, j, k] + (
            1.0 / float(valid_counter)
        ) * damping * (q_nearby - div[i, j, k])
    ...
    def _g2p_apic_kernel(
        ...
        pos = p[i] + vel * dt
        for j in range(3):
            pos[j] = wp.clamp(pos[j], float(boundary), float(ug.shape[j] - boundary))
        p[i] = pos
        u[i] = vel
        c[i] = aff
...
if __name__ == "__main__":
    ...
    if False:
        renderer = wp.render.OpenGLRenderer(
            camera_pos=(1.0, 0.8, 3.5), draw_sky=False, draw_axis=False, vsync=False
        )
    ...
    for _ in range(1024):
        ...
        scene.particles(
            particle_pos, color=(2 / 255, 136 / 255, 210 / 255), radius=0.002
        )
        vertices = np.array([
        ...
        if False:
            ...
            arr = solver.pn_ / res
            renderer.render_points(name="particles", points=arr, radius=0.002)
            renderer.end_frame()
